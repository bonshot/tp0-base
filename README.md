...
Para el último ejercicio tuve que agregar concurrencia con paralelismo para lectura y envío de mensajes entre cliente y server, entonces como bien decía la advertencia de la consigna, `multithreading` en `Python` utiliza el _GIL (Global Interpreter Lock)_ que evita que dos threads se corran al mismo tiempo realmente, entonces tuve que usar `multiprocessing` para poder lograrlo.  
Para comunicarme entre los procesos usé `Queue` de `multiprocessing` para poder enviar mensajes entre los procesos que avisen cuando un handler de cliente terminó de recibir todos los mensajes y así contabilizarlo en el proceso padre del servidor con el fin de ejecutar el sorteo en el momento debido. Además también puedo usar la `Queue` como si de un problema de múltiples productores y un consumidor se tratase, evitando crear nuevas instancias de comunicación porque se comparte la misma entre los procesos a diferencia de los `Pipes`.  
En cuanto a los sockets de los clientes, tuve que guardarme una lista de los sockets al igual que en el ejercicio pasado pero esta vez como los procesos hijos no comparten memoria y luego de recibir todas las bets tengo que hacer el sorteo, para lo que el proceso padre del servidor se encarga de cargar y diferenciar los ganadores de cada agencia para después spawnear nuevos procesos que se encarguen del envío en paralelo de los mensajes de ganadores a cada cliente. Como para esto necesito saber a qué agencia pertenece cada socket de cliente, usé un diccionario que mapea los sockets a las agencias que los crearon, esto se hace en el primer mensaje recibido de parte de cada cliente, se extrae la IP y número de agencia correspondiente a ese cliente y así se inicializa el diccionario mappeando el número a la IP del cliente para que como resultado tenga pares de la forma `{ID_AGENCIA: SOCKET_CLIENTE}`.  
Análogo a los ejercicios anteriores, para poder probar esto podemos ver los logs y darnos cuenta que en la recepción de los mensajes en el server, las agencias están intercaladas en lugar de todas las de una misma agencia en serie, lo que nos indica que se están recibiendo de forma paralela. Y cuando se realiza el sorteo en las últimas líneas de log podemos ver cómo los clientes muestran la cantidad de ganadores de su agencia y el servidor cierra los recursos de manera _graceful_.
